...
class LLMJEmodel(nn.Module):
    def __init__(self, pretrainedModel):
        ...
        self.RE_W_Q = nn.Linear(1536, 768, bias=False)
        self.RE_W_K = nn.Linear(1536, 768, bias=False)
        self.RE_W_V = nn.Linear(1536, 768, bias=False)
        self._norm_fact = 1 / math.sqrt(512)
        self.RE_norm_fact = 1 / math.sqrt(768)
        self.fc1head = nn.Linear(512, 2)
        self.fc1tail = nn.Linear(512, 2)
        self.RE_fc1head = nn.Linear(768, 2)
        self.RE_fc1tail = nn.Linear(768, 2)
        self.Sigmoid1head = nn.Sigmoid()
        self.Sigmoid1tail = nn.Sigmoid()
        self.RE_Sigmoid2head = nn.Sigmoid()
        self.RE_Sigmoid2tail = nn.Sigmoid()
    ...
    def REmodel(self,preTrainOut,RE1Pos,RE1AddContext,contextSingleREnum):
        ...
        RE1AddContextSingle = torch.unsqueeze(RE1AddContextSingle, dim=0)

        selfREAttOut = self.self_RE_Attention(RE1AddContextSingle)
        ...
        return REfc1headOut,REfc1tailOut
    ...
    def forward(self, inputTextData,Ner1Gather,trainModel='GPU',train =0):
        if self.tuneing:
                ...
                for RE1PosNum in range(len(NERheadGrather[contextSingleREnum])):
                    ...
                    if train == 1:
                        if RE1Pos in Ner1Gather[contextSingleREnum]:
                            ...
                            REfc1headOut,REfc1tailOut = self.REmodel(preTrainOut,RE1Pos,RE1AddContext,contextSingleREnum)
        ...
        return headOut, tailOut, NERheadGrather, REpreHeadResult, REpreTailResult,inputTextData['attention_mask'],RE2LabelIndex
...
if __name__ == '__main__':
    ...
    for i, (a1, a2, a3, a4, a5,a6) in enumerate(a):
        b0,b1, b2, b3, b4, b5,b6 = testModel(a1,a4,'CPU',1)
        # print(b5)
        print(b5)
        break
