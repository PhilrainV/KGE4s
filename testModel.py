...
def computeModedl(enconderToken, testModel, allTestData, device):
    ...
    for n in range(len(allTestData)):
        ...
        preNerHead, preNerTail, preNERheadGrather, preREhead, preREtail, attPos,_ = testModel(inputEncoder,0)
        predREgather = []
        NER2PosGrather = findREpos1(preREhead, preREtail)

        print(preNERheadGrather)
...
if __name__ == '__main__':
    ...
    CasrelState_dict = torch.load('model/LLMJEgj(100E).pth', map_location='cuda:0')   #JPEA40.pth
