...
def train(selfTrainData, Epochs):
    ...
    for epoch in tqdm(range(Epochs)):
        ...
        for step, (inputTextData, NerPosHeadLabel, NerPosTailLabel, RE1Pos, RE2PosLabel, context) in enumerate(
            ...
            preNerHead, preNerTail, preNERheadGrather, preREhead, preREtail, attPos, RE2LabelIndex = model(
                inputTextData, NerPosHeadLabel, NerPosTailLabel, RE1Pos, "GPU", 1)
            ...
            try:
                for RE2LabelIndexSingle in RE2LabelIndex:
                    ...
                    preREheadLabel = torch.cat((preREheadLabel, RE2PosLabel[BatchNum][RE2LabelNum][0].to(device)),
                                               dim=0)
                    preREtailLabel = torch.cat((preREtailLabel, RE2PosLabel[BatchNum][RE2LabelNum][1].to(device)),
                                               dim=0)
            ...
            try:
                lossRE = (criterion(preREhead.reshape(-1, 2)[REbackPosSelectFlatten],
                                    preREheadLabel[REbackPosSelectFlatten]) \
                          + criterion(preREtail.reshape(-1, 2)[REbackPosSelectFlatten],
                                      preREtailLabel[REbackPosSelectFlatten])) / preREpot
                testi1 += 1
            except:
                testi2 += 1
