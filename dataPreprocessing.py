...
def dataProcessing(dataPath, Token):
    ...
    for i in tqdm(range(len(trainLines))):
        ...
        preprocessingAllData.append([linedict['sentText'], NerPosHeadSingleLabel, NerPosTailSingleLabel, RE1Pos, RE2PosLabel])
...
def REselfDataLoader(selfData, enconderModel='hfl/rbt6', batchSize=3):
    enconderToken = AutoTokenizer.from_pretrained(enconderModel)
    selfDataset = dataProcessing(selfData, enconderToken)
...
if __name__ == '__main__':
    selfDataPath = './data/ATMtestReLabel.json'
    enconderModel = 'hfl/rbt6'
    a = REselfDataLoader(selfDataPath, batchSize=3)
